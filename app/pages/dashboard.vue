  ...
  <main class="min-h-screen bg-gray-50 font-sans text-slate-800">
    ...
    <nav class="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200 shadow-sm transition-all">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <div class="hidden md:flex items-center gap-8">
            <NuxtLink v-for="item in menuItems" :key="item.label" :to="item.link" class="text-sm font-semibold text-slate-600 hover:text-blue-900 transition-colors">
              {{ item.label }}
            </NuxtLink>
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden border-t border-slate-100 bg-white shadow-inner py-2 transition-all">
        <NuxtLink v-for="item in menuItems" :key="item.label" :to="item.link" @click="isMobileMenuOpen = false" class="block px-6 py-3 text-sm font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-900 transition-all">
          {{ item.label }}
        </NuxtLink>
